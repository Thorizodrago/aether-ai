...
const SplashCursor = ({
    ...
    `
		...
		const IDLE_TIMEOUT = 3000; // Pause animation after 3 seconds of no interaction
		let lastFrameTime = 0;

		function updateFrame(currentTime = Date.now()) {
			// Check if idle
			if (currentTime - lastInteractionTime > IDLE_TIMEOUT) {
				isIdle = true;
			}

			// If idle, run at very low frame rate
			if (isIdle) {
				if (currentTime - lastFrameTime < FRAME_INTERVAL * 4) {
					requestAnimationFrame(updateFrame);
					return;
				}
			} else {
				// Frame rate limiting for active state
			...
			// Skip frames to reduce load when not idle
			if (!isIdle) {
				frameCount++;
				if (frameCount % 2 !== 0) {
					requestAnimationFrame(updateFrame);
